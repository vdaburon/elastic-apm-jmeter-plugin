...
package io.github.vdaburon.jmeterplugins.elasticapm.gui;
...
public class ElasticApmIntegrateGui extends AbstractAction implements
    ...
    private static final Logger log = LoggerFactory.getLogger(ElasticApmIntegrateGui.class);
    ...
    public ElasticApmIntegrateGui() {
        super();
        log.debug("Creating ElasticApmIntegrateGui");
    }
    ...
    private void setupContentPane() {
        ...
        BorderFactory.createEtchedBorder(), "ELASTIC APM Configuration"));
...
//        log.info("command=" + command);
        ...
        if (command.equals(ACTION_MODIFY) || command.equals(ACTION_MODIFY_AND_LOAD)) {
            ...
            if (!fFileIn.canRead()) {
            	labelStatus.setText("Tool ELASTIC APM Integration Finished KO, CAN'T READ jmx fileIn = " + fileIn);
            ...
            if (sAction.equalsIgnoreCase(ElasticApmJMeterManager.ACTION_ADD)) {
            	sAction = ElasticApmJMeterManager.ACTION_ADD;
            }
            
            if (sAction.equalsIgnoreCase(ElasticApmJMeterManager.ACTION_REMOVE)) {
            	sAction = ElasticApmJMeterManager.ACTION_REMOVE;
            }
            ...
            try {
                ...
                if (!fFileOut.canRead()) {
     	 		...
     	 		labelStatus.setText("Tool ELASTIC APM Integration Finished OK, ACTION = " + sAction + ", fileOut=" + fileOut);
